...
def fast_non_dominated_sort_three(values1, values2, values3):
    S = [[] for i in range(0, len(values1))]
    front = [[]]
    n = [0 for i in range(0, len(values1))]
    rank = [0 for i in range(0, len(values1))]
...
def crowding_distance_three(values1, values2, values3, front):
    distance = [0 for i in range(0, len(front))]
    sorted1 = sort_by_values(front, values1[:])
    sorted2 = sort_by_values(front, values2[:])
...
def NSGA_2_three(target1, target2, target3, N):
    '''
    最大化三个目标
    target:适应度列表
    N:保留的个数
    ...
    function1_values2 = target1
    function2_values2 = target2
    function3_values3 = target3
    non_dominated_sorted_solution2 = fast_non_dominated_sort_three(function1_values2[:], function2_values2[:], function3_values3[:])  # index
    crowding_distance_values2 = []
    for i in range(0, len(non_dominated_sorted_solution2)):
        crowding_distance_values2.append(
            crowding_distance_three(function1_values2[:], function2_values2[:], function3_values3[:], non_dominated_sorted_solution2[i][:]))
    new_solution = []
    for i in range(0, len(non_dominated_sorted_solution2)):
        non_dominated_sorted_solution2_1 = [
            index_of(non_dominated_sorted_solution2[i][j], non_dominated_sorted_solution2[i]) for j in
