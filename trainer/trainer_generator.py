...
from utils.nsga import NSGA_2, NSGA_2_three
...
class GenTrainer():
    def __init__(self, args, gen_net, dis_net, gen_optimizer, dis_optimizer, train_loader, gan_alg, dis_genotype,
        ...
        # 加载教师模型
        self.teacher_gen_net = deepcopy(gen_net)
        # teacher_gen_net.load_state_dict(torch.load(args.teacher_gen_ckpt))
        self.teacher_gen_net.eval()

    def train(self, epoch, writer_dict, fid_stat, MetaN, gen_kd_optimizer, optimizer, MetaMN, prioritized_board, schedulers=None):
        ...
        genotype_G = np.stack([self.gan_alg.search() for i in range(num_g)],    # 均匀采样了7个生成器
                              axis=0)
        for iter_idx, (imgs, _) in enumerate(tqdm(self.train_loader)):
            ...
            n_i = iter_idx % num_g
            fake_imgs = gen_net(z, genotype_G[n_i])[0].detach()
            ...
            if global_steps % self.args.n_critic == 0:
                ...
                for k in range(num_g):
                    ...
                    random_cand = genotype_G[k]

                    # update meta matching networks
                    MetaMN.run_update(gen_z, random_cand, gen_net, gen_kd_optimizer, dis_net, MetaN, optimizer,
                                      prioritized_board, epoch, iter_idx)

                    # get_best_teacher
                    if prioritized_board.board_size() > 0:
                        meta_value, teacher_cand = prioritized_board.select_teacher(gen_net, MetaN, random_cand)

                    student_output, student_intermediates = gen_net(gen_z, random_cand)
                    fake_validity = dis_net(student_output)
                    g_loss_adv = -torch.mean(fake_validity)
                    if prioritized_board.board_size() == 0 or epoch <= self.args.META_STA_EPOCH:
                        g_loss = g_loss_adv
                        kd_loss, teacher_output, teacher_cand = None, None, None
                    else:
                        # get soft label from teacher cand
                        with torch.no_grad():
                            teacher_output, _ = gen_net(gen_z, teacher_cand)
                            teacher_output.detach()
                            soft_label = F.softmax(teacher_output, dim=1)

                        kd_loss = KnowledgeDistillationLoss(student_output, soft_label)
                        kd_loss = meta_value * kd_loss

                        g_loss_adv = (2 - meta_value) * g_loss_adv
                        g_loss = (kd_loss + g_loss_adv) / 2

                    g_loss.backward()

                    if global_steps % self.args.PRIORITIZED_BOARD_UPDATE_ITER == 0 and epoch > self.args.META_STA_EPOCH:
                        is_value, is_std, fid_value = self.validate(random_cand, fid_stat)
                        prioritized_board.update_prioritized_board(gen_z, teacher_output, student_output, epoch, is_value,
                                                                   fid_value, random_cand)
                ...
                # learning rate
                if schedulers:
                    gen_scheduler, dis_scheduler, gen_kd_scheduler = schedulers
                    g_lr = gen_scheduler.step(global_steps)
                    d_lr = dis_scheduler.step(global_steps)
                    g_kd_lr = gen_scheduler.step(global_steps)
                    writer.add_scalar('LR/g_lr', g_lr, global_steps)
                    writer.add_scalar('LR/d_lr', d_lr, global_steps)
                    writer.add_scalar('LR/g_kd_lr', g_kd_lr, global_steps)
            ...
            if gen_step and iter_idx % self.args.print_freq == 0:
                if kd_loss:
                    tqdm.write(
                        '[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [Adv loss: %f] [KD loss: %f]' %
                        (epoch, self.args.max_epoch_D, iter_idx % len(self.train_loader), len(self.train_loader),
                         d_loss.item(), g_loss.item(), g_loss_adv.item(), kd_loss.item()))
                else:
                    tqdm.write(
    ...
    def my_search_evolv2(self, parents, fid_stat, num):
        ...
        self.genotypes = genotypes[keep]
        return genotypes[keep], genotypes[selected], \
            np.mean(is_values[keep]), np.mean(fid_values[keep]), \
            is_values[keep], fid_values[keep]

    def my_search_evol(self, parents, fid_stat, num):
        offsprings = self.gen_offspring(parents)
        # 合并种群
        genotypes = np.concatenate((parents, offsprings), axis=0)
        is_values, fid_values, params, is_fid_values = np.zeros(len(genotypes)), np.zeros(
            len(genotypes)), np.zeros(len(genotypes)), np.zeros(len(genotypes))
        keep_N, selected_N = len(offsprings), self.args.num_selected
        # 评估候选架构
        for idx, genotype_G in enumerate(tqdm(genotypes)):
            encode_G = self.gan_alg.encode(genotype_G)
            # 将架构编码 encode_G 存储在历史种群 self.historical_population 中，以避免重复计算。
            if encode_G in self.historical_population:
                data = self.historical_population[encode_G]
                is_value, fid_value, param_szie = data[0], data[1], data[2]
            else:
            ...
            fid_values[idx] = fid_value
            params[idx] = param_szie
            is_fid_values[idx] = 0.5 * is_value / 5 - 0.5 * fid_value / 90
            # 更新最佳性能
            if is_value > self.best_is_evo:
                self.best_is_evo = is_value
                file_path = os.path.join(self.args.path_helper['ckpt_path'],
        ...
        logger.info(
            f'mean_IS: {round(avg_is, 3)}, mean_FID: {round(avg_fid, 3)}, max_IS: {round(np.max(is_values), 3)}, min_FID: {round(np.min(fid_values), 3)},mean_params: {round(avg_params, 3)}@ evolutionary_algebra: {num}.')
        # 多目标进化算法
        keep = NSGA_2(is_fid_values.tolist(), (-params).tolist(), keep_N)
        selected = NSGA_2(is_fid_values.tolist(), (-params).tolist(), selected_N)
    ...
    def my_search_evol_2(self, parents, fid_stat, num):
        offsprings = self.gen_offspring(parents)
        # 合并种群
        genotypes = np.concatenate((parents, offsprings), axis=0)
        is_values, fid_values, params, is_fid_values = np.zeros(len(genotypes)), np.zeros(
        ...
        for idx, genotype_G in enumerate(tqdm(genotypes)):
            ...
            fid_values[idx] = fid_value
            params[idx] = param_szie
            is_fid_values[idx] = 0.5 * is_value / 8 - 0.5 * fid_value / 50
            # 更新最佳性能
            if is_value > self.best_is_evo:
                self.best_is_evo = is_value
                file_path = os.path.join(self.args.path_helper['ckpt_path'],
    ...
    def my_search_evol_3(self, parents, fid_stat, num):
        offsprings = self.gen_offspring(parents)
        # 合并种群
        genotypes = np.concatenate((parents, offsprings), axis=0)
        ...
        logger.info(
            f'mean_IS: {round(avg_is, 3)}, mean_FID: {round(avg_fid, 3)}, max_IS: {round(np.max(is_values), 3)}, min_FID: {round(np.min(fid_values), 3)},mean_params: {round(avg_params, 3)}@ evolutionary_algebra: {num}.')
        # 多目标进化算法
        keep = NSGA_2_three(is_values.tolist(), (-fid_values).tolist(), (-params).tolist(), keep_N)
        selected = NSGA_2_three(is_values.tolist(), (-fid_values).tolist(), (-params).tolist(), selected_N)
    ...
    def validate(self, genotype_G, fid_stat):
        ...
        with torch.no_grad():
            for iter_idx in tqdm(range(eval_iter), desc='sample images'):
                ...
                gen_imgs = (gen_net(z, genotype_G)[0] + 1) / 2
