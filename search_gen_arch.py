...
from archs.MetaMatchingNetwork import MetaMatchingNetwork
from archs.MetaNet import MetaNet
from archs.PrioritizedBoard import PrioritizedBoard
...
def main():
    ...
    # gen_kd_optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, gen_net.parameters()),
    #                                  args.g_lr, (args.beta1, args.beta2))
    gen_kd_optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, gen_net.parameters()),
                                       args.g_kd_lr)
    ...
    gen_kd_scheduler = LinearLrDecay(gen_kd_optimizer, args.g_kd_lr, 0.0, 0, max_iter_D)
    ...
    basemodel_metanet = MetaNet()
    MetaN = torch.nn.DataParallel(
        basemodel_metanet, device_ids=args.gpu_ids).cuda(args.gpu_ids[0])
    MetaMN = MetaMatchingNetwork(args)

    optimizer = torch.optim.SGD(MetaN.parameters(), lr=0.01, momentum=0.9, weight_decay=1e-4)

    # 初始化优先级板（PrioritizedBoard），用于存储和管理超网络的优先级信息。
    # initialize prioritized board
    choice_num = 10
    prioritized_board = PrioritizedBoard(args, CHOICE_NUM=choice_num)

    # search genarator
    for epoch in tqdm(range(int(start_epoch), int(200)), desc='training the supernet_G:'):
        lr_schedulers = (gen_scheduler, dis_scheduler, gen_kd_scheduler) if args.lr_decay else None
        trainer_gen.train(epoch, writer_dict, fid_stat, MetaN, gen_kd_optimizer, optimizer, MetaMN, prioritized_board, schedulers = lr_schedulers)
        if epoch == 200:
            break

        # if epoch % 5 == 0:
        #     population = np.stack([gan_alg.search() for i in range(20)], axis=0)
        #     for ii in tqdm(range(20), desc='search genearator using evo alg'):
        #         population, pop_selected, is_mean, fid_mean, _, _ = trainer_gen.my_search_evolv2(population, fid_stat, ii)

        if epoch == args.warmup * args.n_critic:  # 开始修剪
